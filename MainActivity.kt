...
class MainActivity : ComponentActivity() {
    ...
    private lateinit var appUpdateManager: com.example.data.update.AppUpdateManager
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        appUpdateManager = AppModule.provideAppUpdateManager(applicationContext)

        // Asynchronous cold start GitHub release check (non-blocking, fails silently on network errors)
        appUpdateManager.checkOnStartup(lifecycleScope)
            ...
            if (activeCrashLog != null) {
                ...
                SwarMusicTheme(
                    ...
                    Surface(
                        ...
                        // In-App Update Prompt Dialog (reappears on cold start if outdated)
                        val updateDialogState by appUpdateManager.updateDialogState.collectAsStateWithLifecycle()
                        if (updateDialogState.isVisible && updateDialogState.release != null) {
                            val release = updateDialogState.release!!
                            AppUpdateDialog(
                                release = release,
                                isDownloading = updateDialogState.isDownloading,
                                onUpdateNow = {
                                    appUpdateManager.startUpdate(this@MainActivity, release)
                                },
                                onDismissSession = {
                                    appUpdateManager.dismissDialogForSession()
                                },
                                onOpenInBrowser = {
                                    val targetUrl = release.apkDownloadUrl ?: release.htmlUrl
                                    appUpdateManager.openInBrowser(this@MainActivity, targetUrl)
                                }
                            )
                        }
                    }
    ...
    override fun onDestroy() {
        ...
        if (::appUpdateManager.isInitialized) {
            appUpdateManager.unregisterDownloadReceiver(applicationContext)
        }
